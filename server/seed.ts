...
const seedData = async()=>{
    try {
        ...
        console.log("DB Connected. Clearing DB...");
    ...
    },
    {
        ...
        const updatedRestaurantsData = restaurantsData.map((restaurant, index) => {
            const {...restInfo} = restaurant;
            return{
                ...restInfo,
                owner: ownerUser._id,
                status: "approved",
                totalSeats: 20 + index * 5,
            }
        })
        await Restraunt.insertMany(updatedRestaurantsData);
        console.log("Restaurants inserted successfully. Disconnecting from MongoDB...");
